...
RSpec.describe 'Webhooks::Billables', type: :request do
  Sidekiq::Testing.fake!
  path '/webhooks/billable/:token' do
    ...
    post 'handle' do
      response '200', 'OK' do
        ...
        describe 'webhook type' do
          context 'when the web hook type starts with subscription' do
            let(:expected_job_arguments)  do
              [
                ['subscription_event', '{"some":"content"}']
              ]
            end
            it 'triggers a SubscriptionJob' do |example|
              expect { run example }.to change(Billable::Webhook::SubscriptionJob.jobs, :size).by(1)
              enqueued_job_arguments = Billable::Webhook::SubscriptionJob.jobs.pluck('args')
