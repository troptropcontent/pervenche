class RadioTilesInput < SimpleForm::Inputs::Base
  ...
  def tile(collection_item)
    ...
    options[:disabled] ? fake_tile(title, subtitle, value) : real_tile(title, subtitle, value, checked:)
  end

  def real_tile(title, subtitle, value, checked:)
    template.content_tag(:label, id: value, class: 'form__input__tiles__tile') do
      [
        @builder.input_field(attribute_name, type: 'radio', class: 'visually-hidden', value:, checked:),
  ...
  def fake_tile(title, subtitle, _value)
